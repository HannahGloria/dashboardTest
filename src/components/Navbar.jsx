...
// ...existing code...
import logo from '../assets/img/cryptocoinsLogo(2).png';
import avatar from '../assets/img/rosita.jpg'

export default function Navbar() {
  const [mobileOpen, setMobileOpen] = useState(false);

  return (
    <nav className="relative bg-stone-950 after:pointer-events-none after:absolute after:inset-x-0 after:bottom-0 after:h-px after:bg-white/10">
        ...
        <div className="relative flex h-16 items-center justify-between">
          ...
          <div className="flex flex-1 items-center justify-center sm:items-stretch sm:justify-start">
            <div className="flex shrink-0 items-center">
              <img src={logo} alt="CryptoCoins by Leonor" className="h-18 w-auto" />
            </div>
            <div className="hidden sm:ml-6 sm:block m-auto">
              <div className="flex space-x-4 m-auto text-center">
                ...
                <a href="./MarketChart.jsx" className="rounded-md px-3 py-2 text-sm font-medium text-gray-300 hover:bg-white/5 hover:text-white">Tendencias</a>
                <a href="#" className="rounded-md px-3 py-2 text-sm font-medium text-gray-300 hover:bg-white/5 hover:text-white">Team de Cryptocoins</a>
                <a href="./RadialPrice.jsx" className="rounded-md px-3 py-2 text-sm font-medium text-gray-300 hover:bg-white/5 hover:text-white">Ventas</a>
              </div>
            </div>
          </div>
          ...
          <div className="absolute inset-y-0 right-0 flex items-center pr-2 sm:static sm:inset-auto sm:ml-6 sm:pr-0">
            ...
            <div className="relative ml-3">
              <button className="relative flex rounded-full focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-500">
                ...
                <img src={avatar} alt="User avatar" className="h-12 w-12 rounded-full bg-gray-800 outline -outline-offset-1 outline-white/10" />
              </button>
              {/* Si quieres un dropdown real, integra tu librería de UI o añade lógica adicional */}
            </div>
          </div>
        </div>
      </div>
      ...
      {mobileOpen && (
        <div id="mobile-menu" className="block sm:hidden">
          <div className="space-y-1 px-2 pt-2 pb-3">
            ...
            <a href="#" className="block rounded-md px-3 py-2 text-base font-medium text-gray-300 hover:bg-white/5 hover:text-white">Tendencias</a>
            <a href="#" className="block rounded-md px-3 py-2 text-base font-medium text-gray-300 hover:bg-white/5 hover:text-white">Team</a>
            <a href="#" className="block rounded-md px-3 py-2 text-base font-medium text-gray-300 hover:bg-white/5 hover:text-white">Ventas</a>
          </div>
        </div>
      )}
    </nav>
